...
export class DateUtils{
   ...
   */
    static async parseDatesInFields(fields: Record<string, any>, originalPrompt: string): Promise<Record<string, any>> {
      ...
      if (fields.endDate && typeof fields.endDate === 'string') {
        ...
        if (parsedDate) {
          ...
          if (parsedFields.startDate) {
            parsedFields.actualDuration = await DateUtils.calculateDuration(
     ...
     */
      static async calculateDuration(startDate: Date, endDate: Date): Promise<number> {
